...
<h3>What is MPFM</h3>
<p>
  一个学生用ai写的大作业。架构一坨，各种设计混用且不彻底，不建议阅读源代码。
...
## 碎碎念
AI写的太TM难看了，一坨。
一不注意去review，就变成了这么一坨，改都难改。
后端TM的我强调很多次协议功能只能再内部实现，然后通过统一接口调用。
结果vibe coding过程，没注意，一路同意。导致部分判断逻辑上浮到了控制编排里面。
即，if (是local) 调用local的xxx功能。 elif (是eftp) 调用sftp的xxx功能
